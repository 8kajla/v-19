...
class TraderPolicyScheduler:
    ...
    def __init__(self, behavior, seed=20260831):
        self.rng = random.Random(seed)
        raw = {str(x["fine_band"]): float(x["trade_share"])
               for x in behavior.get("fine_bands", [])}
        self.capital_targets = {str(x["fine_band"]): float(x["notional_share"])
                                for x in behavior.get("fine_bands", [])}
        ...
        self.trade_targets = {b: v / total_trade_share for b, v in raw.items()}
    ...
    def observe(self, band, notional):
        if band not in self.trade_counts: return
        self.trade_counts[band]+=1
        self.capital[band]+=max(0.0,float(notional))

    def restore(self, trades, fine_band_fn):
        self.trade_counts={b:0 for b in self.bands}; self.capital={b:0.0 for b in self.bands}
        for t in trades or []:
            if t.get("action")!="BUY": continue
            band=t.get("fine_band")
            if band not in self.trade_counts:
                try: band,_=fine_band_fn(float(t.get("signal_price",t.get("price"))))
                except Exception: continue
            if band in self.trade_counts:
                self.trade_counts[band]+=1; self.capital[band]+=max(0.0,float(t.get("notional",t.get("cost",0))))

    def projected_state(self, band, notional):
        nt=sum(self.trade_counts.values())+1; nc=sum(self.capital.values())+max(0,float(notional))
        tr={b:(self.trade_counts[b]+(b==band))/nt for b in self.bands}
        ca={b:(self.capital[b]+(float(notional) if b==band else 0))/nc if nc else 0 for b in self.bands}
        return tr,ca

    def _need(self, band, notional):
        tr, ca = self.projected_state(band, notional)
        td = (self.trade_targets[band] - tr[band]) / max(self.trade_targets[band], 1e-9)
        cd = (self.capital_targets[band] - ca[band]) / max(self.capital_targets[band], 1e-9)
    ...
    def choose_band(self, candidates, allow_over_quota=False):
        ...
        for band, rows in by.items():
            ...
            row = max(rows, key=lambda x: self._need(band, float(x.get("target", 0.0))))
            scored.append((self._need(band, float(row.get("target", 0.0))),
                           -BAND_INDEX.get(band, 999), band))
        scored.sort(reverse=True)
        return scored[0][2]

    def shares(self):
        nt=sum(self.trade_counts.values()); nc=sum(self.capital.values())
        return {"trade":{b:self.trade_counts[b]/nt if nt else 0 for b in self.bands},
                "capital":{b:self.capital[b]/nc if nc else 0 for b in self.bands}}

    def target_report(self):
        a=self.shares(); return {b:{"target_trade_share":self.trade_targets[b],"actual_trade_share":a["trade"][b],"target_capital_share":self.capital_targets[b],"actual_capital_share":a["capital"][b]} for b in self.bands}
...
class CapitalFirstStrategy:
    ...
    def _candidate(
        ...
        return {
            ...
            "reason": (
                ...
                f"behavioral_target=${target:.2f} entry_count={int(entries)} "
    ...
    def build_candidates_for_market(
        ...
        up_exec_depth = up_ask_depth if up_ask_depth is not None else up_depth
        down_exec_depth = down_ask_depth if down_ask_depth is not None else down_depth
        candidates = [
            c for c in (
                self._candidate(
                    m, "Up", up_bid, up_ask, up_exec_depth, up_history,
                    now, thesis_side, market_entry_count, seconds_since_first_entry,
                ),
                self._candidate(
                    m, "Down", down_bid, down_ask, down_exec_depth, down_history,
                    now, thesis_side, market_entry_count, seconds_since_first_entry,
                ),
            )
            if c is not None
    ...
    def decide(
        ...
        reason = (
            ...
            f"same_side={best['same_side']} behavioral_target=${target:.2f} entry_count={market_entry_count} "
