...
scan_metrics = {"scannable": 0, "candidates": 0, "executable": 0, "last_attempt_ts": None}
...
MAX_MARKET_EXPOSURE = max(0.0, float(os.getenv("MAX_MARKET_EXPOSURE", "300")))
MAX_ASSET_EXPOSURE = max(0.0, float(os.getenv("MAX_ASSET_EXPOSURE", "300")))
MAX_ORDER_USD = max(0.0, float(os.getenv("MAX_ORDER_USD", "300")))
MIN_PAPER_FILL_USD = max(0.01, float(os.getenv("MIN_PAPER_FILL_USD", "0.10")))

BOOK_EXECUTOR = ThreadPoolExecutor(max_workers=BOOK_WORKERS, thread_name_prefix="v19-book")
...
def position_exposure(*, asset=None, market=None, condition=None):
    ...
    for p in ledger.positions.values():
        if condition is not None and str(p.get("condition")) != str(condition):
            continue
        if asset is not None and str(p.get("asset", "")).upper() != str(asset).upper():
            continue
        if market is not None and str(p.get("market", "")) != str(market):
            continue
...
def _empty_market_entry_state():
    return {
        "count": 0, "first_ts": None, "previous_ts": None,
        "side": None, "price": None, "burst_position": 0,
    }
...
def _observe_market_entry(condition, trade):
    ...
    try:
        ...
        return
    state = market_entry_index.setdefault(condition, _empty_market_entry_state())
    previous = state.get("previous_ts")
    if previous is None:
        burst = 1
        first_ts = ts
    else:
        gap = max(0.0, ts - float(previous))
        burst = int(state.get("burst_position") or 1) + 1 if gap <= BURST_GAP_SECONDS else 1
        first_ts = state.get("first_ts") if state.get("first_ts") is not None else ts
    state.update({
...
def rebuild_market_entry_index(trades):
    ...
    last_trade.clear()
    buys = [t for t in trades if t.get("action") == "BUY" and t.get("condition")]
    buys.sort(key=lambda t: float(t.get("ts", 0.0)))
    for trade in buys:
        _observe_market_entry(trade.get("condition"), trade)


def market_entry_state(condition, now):
    state = market_entry_index.get(str(condition))
    if not state:
        return {
    ...
    return {
        ...
        "seconds_since_first": max(0.0, float(now) - float(first)) if first is not None else 0.0,
        "seconds_since_previous": max(0.0, float(now) - float(previous)) if previous is not None else None,
        "side": state.get("side"),
        "price": state.get("price"),
        "burst_position": int(state.get("burst_position") or 0),
    }
...
def resolve_pending(now):
    for condition in list(resolution_markets):
        market = resolution_markets.get(condition) or markets.get(condition)
...
def record_filled_trade(market, token, signal, result, meta, now, state):
    ...
    _observe_market_entry(market["condition"], trade)
...
def report(books):
    ...
    log.info(
        "SIGNAL SCAN | scannable=%d candidates=%d executable=%d next_trade_in=%.2fs",
        int(scan_metrics.get("scannable", 0)),
        int(scan_metrics.get("candidates", 0)),
        int(scan_metrics.get("executable", 0)),
        max(0.0, float(next_trade_at) - now),
    )
...
def candidate_execution_key(candidate, now=None):
    ...
    if now is None:
        now = time.time()
    condition = str((candidate.get("_market") or {}).get("condition", ""))
    last = last_trade.get(condition) if condition else None
    age = float("inf") if last is None else max(0.0, float(now) - float(last))
    return (
        age,
        float(candidate.get("trajectory_likelihood", 0.0)),
        1 if candidate.get("same_side") else 0,
        float(candidate.get("depth") or 0.0),
        -float(candidate.get("bid") or 0.0),
    )


def prepare_immediate_candidate(candidate, strategy, market, state):
    """Keep the behavioral signal band; CLOB is execution-only.

    The candidate's band is the trader-replication segment. The live ask may
    be in another price band; that must not rewrite the behavioral decision.
    """
    ask=candidate.get("ask")
    if ask is None:
        candidate["_clob_executable"]=False; return candidate
    try: ask=float(ask)
    except (TypeError,ValueError):
        candidate["_clob_executable"]=False; return candidate
    if not 0.0 < ask < 1.0:
        candidate["_clob_executable"]=False; return candidate
    candidate["signal_band"]=candidate.get("band")
    candidate["signal_bid"]=float(candidate["bid"])
    candidate["execution_band"]=None
    candidate["execution_ask_band"], candidate["execution_regime"]=strategy.fine_band(ask)
    candidate["target"]=strategy.entry_target(
        float(candidate.get("bid",ask)), market=market["asset"], entry_count=state["count"]
    )
...
def choose_executable_band(
    candidates, strategy, allow_emergency=False, now=None, reuse_cooldown_seconds=0.0
):
    """Select the behavioral band first, then expose only executable rows.

    CLOB availability must never choose a different behavioral band.  A band is
    selected from the full candidate set; if that selected band has no current
    ask, this decision is simply unfilled and the next cadence decision starts
    fresh.  The legacy arguments remain for compatibility but have no effect.
    """
    del allow_emergency, reuse_cooldown_seconds
    if not candidates:
        return None, False, []
    now = time.time() if now is None else float(now)
    band = strategy.choose_distribution_band(candidates)
    if band is None:
        return None, False, []
    rows = [c for c in candidates if c.get("band") == band and c.get("_clob_executable")]
    rows.sort(key=lambda c: candidate_execution_key(c, now), reverse=True)
    return band, False, rows

def main():
    global last_disc, next_trade_at, consecutive_errors, scan_metrics

    initialize_runtime()
    startup_check()
    log.info(
        "BOT B | PAPER ONLY | IMMEDIATE CLOB TAKER SIMULATION | "
        "V19 RAW BEHAVIORAL REPLICA | CLOB EXECUTION ONLY"
    )
    ...
    while True:
        try:
            ...
            if now >= next_trade_at:
                ...
                scan_metrics["scannable"] = len(scannable)
                scan_metrics["candidates"] = 0
                scan_metrics["executable"] = 0
                scan_metrics["last_attempt_ts"] = now
                ...
                for m, elapsed, left in scannable:
                    ...
                    ub, ua, ud, uad = pair.get("up", (None, None, 0.0, 0.0))
                    db, da, dd, dad = pair.get("down", (None, None, 0.0, 0.0))
                    if m.get("up"):
                        books[m["up"]] = ub
                    if m.get("down"):
                        books[m["down"]] = db
                    ...
                    scan_metrics["candidates"] += len(candidates)
                    for candidate in candidates:
                        candidate.update(
                            _market=m, _state=state, _elapsed=elapsed,
                            _left=left,
                        )
                        # Inspect the current ask for execution availability.
                        # This does not change the behavioral signal band.
                        prepare_immediate_candidate(candidate, strategy, m, state)
                        if candidate.get("_clob_executable"):
                            scan_metrics["executable"] += 1
                        eligible.append(candidate)

                if eligible:
                    # IMPORTANT: behavioral distribution selection happens BEFORE CLOB
                    # executability is allowed to influence the chosen band.
                    # If the chosen band has no ask, the decision is unfilled;
                    # never substitute another band just because it is liquid.
                    target_band, emergency_selection, band_candidates = choose_executable_band(
                        eligible, strategy, allow_emergency=False, now=now,
                        reuse_cooldown_seconds=0.0,
                    )
                    if emergency_selection:
                        log.info(
                            "SCHEDULER EMERGENCY | band=%s | executable_candidates=%d | reason=all_quota_ineligible",
                            target_band, len(band_candidates),
                        )

                    if band_candidates:
                        # Try more than one market/side inside the chosen band.
                        # The previous build selected one candidate and stopped
                        # after a race-induced CLOB miss. Here a second asset can
                        # immediately satisfy the same strategy band without
                        # changing the trader policy.
                        for best in band_candidates:
                            ...
                            current_total = ledger.total_open_cost()
                            current_asset = position_exposure(asset=market["asset"])
                            current_market = position_exposure(market=market["market"])
                            remaining_total = max(
                                0.0, strategy.max_total_exposure - current_total - reserved
                            )
                            remaining_asset = max(
                                0.0, MAX_ASSET_EXPOSURE - current_asset - reserved
                            )
                            remaining_market = max(
                                0.0, MAX_MARKET_EXPOSURE - current_market - reserved
                            )
                            available = max(0.0, ledger.cash - reserved)
                            notion = min(
                                float(signal["target"]), MAX_ORDER_USD, available,
                                remaining_total, remaining_asset, remaining_market,
                            )

                            if (
                                notion < MIN_PAPER_FILL_USD
                                or best["_left"] <= strategy.hard_cutoff_seconds
                            ):
                                continue
                            ...
                            # `best["band"]` is the behavioral fine band selected
                            # by the replication model. The live ask is execution-only.
                            band = str(best["band"])
                            ...
                            meta = {
                                ...
                                "regime": regime,
                                "fine_band": band,
                                "signal_fine_band": best.get("signal_band"),
                                "execution_mode": "IMMEDIATE_CLOB_TAKER",
                                "target_capital": float(signal["target"]),
                                "strategy_reference_bid": float(best.get("signal_bid", target)),
                            ...
                            # regardless of whether this particular CLOB attempt
                            # filled. Start the gap from the actual CLOB attempt
                            # completion time so a slow network request cannot
                            # make the next trade immediately eligible.
                            sampled_gap = max(0.0, strategy.cadence.sample_gap())
                            configured_gap = max(0.0, strategy.min_trade_gap_seconds)
                            next_trade_at = time.time() + max(sampled_gap, configured_gap)
                            ...
                            research.record_instant_unfilled(
                                order, result.get("reason", "unfilled"), now
                            )
                        else:
                            # Every candidate in the selected distribution band
                            # was rejected/invalid. Keep the scheduler state
                            # untouched and throttle the next scan so a cash or
                            # exposure cap cannot turn the main loop into a
                            # 4-requests/second CLOB poller.
                            log.info(
                                "CLOB BAND MISS | band=%s | candidates=%d | "
                                "reason=no_candidate_filled",
                                target_band, len(band_candidates),
                            )

            if now >= next_trade_at and not execution_attempted:
                # No execution attempt occurred (no candidate, no executable
                # band, or exposure/cash blocked). Re-scan at the order-book
                # cadence instead of spinning at LOOP_SECONDS.
                next_trade_at = time.time() + max(
                    LOOP_SECONDS, ORDERBOOK_SAMPLE_SECONDS
                )
